...
(VehicleUtilization, DriverIdleRollup, DrivingPeriod, etc.) for the
data the fake client yields. Tests assert the fetcher orchestrates
the three Motive endpoints correctly without performing any
transformation: identity equality is used for records flowing
...
def _build_fake_provider_and_client(
    *,
    ...
    the per-window record maps; DRIVING_PERIODS returns the configured
    flat list regardless of window.
...
class TestMotiveUtilizationFetcherSingleDay:
    """Single-day range fetches all three endpoints exactly once."""

    def test_one_key_per_by_date_dict_and_three_calls(self) -> None:
        """Should produce one by-date key per dict and 3 total fetch_all calls."""
...
class TestMotiveUtilizationFetcherMultiDay:
    """Multi-day range loops per-day endpoints; full-range driving_periods once."""

    def test_three_day_range_keys_and_call_count(self) -> None:
        """Should fan out 3 days x 2 per-day endpoints + 1 driving_periods = 7."""
...
class TestMotiveUtilizationFetcherEndpointAndParameterShapes:
    ...
    def _fetch_and_partition_calls(
        ...
        """Run a 2-day fetch and return (vehicle_calls, driver_calls, period_calls)."""
        ...
        return vehicle_calls, driver_calls, period_calls
    ...
    def test_endpoint_identity_per_call_class(self) -> None:
        ...
        vehicle_calls, driver_calls, period_calls = self._fetch_and_partition_calls()
    ...
    def test_vehicle_utilization_params_are_utc_datetimes(self) -> None:
        ...
        vehicle_calls, _, _ = self._fetch_and_partition_calls()
    ...
    def test_driver_utilization_params_are_utc_datetimes(self) -> None:
        ...
        _, driver_calls, _ = self._fetch_and_partition_calls()
    ...
    def test_driving_periods_params_are_bare_dates(self) -> None:
        ...
        _, _, period_calls = self._fetch_and_partition_calls()
